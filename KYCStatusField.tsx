...
// PATCH: begin useLookupOptions in KYCStatusField
import { useLookupOptions } from '@/features/lookups/hooks'
// PATCH: end useLookupOptions in KYCStatusField

interface KYCStatusFieldProps {
  clientId: string
}

export function KYCStatusField({ clientId }: KYCStatusFieldProps) {
  // PATCH: begin useLookupOptions in KYCStatusField component
  const { options: kycOptions } = useLookupOptions("kycStatus");
  // PATCH: begin log lookup options source
  console.debug("[KYC Select] options from hook:", kycOptions);
  // PATCH: end log lookup options source
  const KYC_STATUS_OPTIONS = [
    { value: '', label: 'Not Set' },
    ...kycOptions
  ];
  // PATCH: end useLookupOptions in KYCStatusField component
  
  const dispatch = useDispatch<AppDispatch>()
  const entity = useSelector(selectEntityById(clientId))
  const current = entity?.kycStatus || ''
  ...
  const getCurrentDisplayValue = () => {
    const option = KYC_STATUS_OPTIONS.find(opt => (opt as any).value === current)
