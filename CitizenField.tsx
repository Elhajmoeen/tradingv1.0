...
interface CitizenFieldProps {
  clientId: string
}

const CITIZEN_STATUS_OPTIONS = [
  { value: '', label: 'Not Set' },
  ...CITIZEN_STATUS.map(status => ({ value: status, label: status }))
]

export function CitizenField({ clientId }: CitizenFieldProps) {
  const dispatch = useDispatch<AppDispatch>()
  const entity = useSelector(selectEntityById(clientId))
  const current = entity?.citizen || ''
  ...
  const getCurrentDisplayValue = () => {
    const option = CITIZEN_STATUS_OPTIONS.find(opt => opt.value === current)
      ...
      <div className="relative flex-1">
        ...
        {isDropdownOpen && (
          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
            <div className="py-1">
              {CITIZEN_STATUS_OPTIONS.map((option) => (
                <button
                  key={option.value}
