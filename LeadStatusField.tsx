...
// PATCH: begin useLookupOptions in LeadStatusField
import { useLookupOptions } from '@/features/lookups/hooks'
// PATCH: end useLookupOptions in LeadStatusField

interface LeadStatusFieldProps {
  clientId: string
}

export function LeadStatusField({ clientId }: LeadStatusFieldProps) {
  // PATCH: begin useLookupOptions in LeadStatusField component
  const { options: leadOptions } = useLookupOptions("leadStatus");
  // PATCH: begin log lookup options source
  console.debug("[Lead Select] options from hook:", leadOptions);
  // PATCH: end log lookup options source
  const LEAD_STATUS_OPTIONS = [
    { value: '', label: 'Not Set' },
    ...leadOptions
  ];
  // PATCH: end useLookupOptions in LeadStatusField component
  
  const dispatch = useDispatch<AppDispatch>()
  const entity = useSelector(selectEntityById(clientId))
  const current = entity?.leadStatus || ''
  ...
  const getCurrentDisplayValue = () => {
    const option = LEAD_STATUS_OPTIONS.find(opt => (opt as any).value === current)
