...
import { selectEntityById, updateGender } from '@/state/entitiesSlice'
...
export function GenderField({ clientId }: GenderFieldProps) {
  const dispatch = useDispatch<AppDispatch>()
  const entity = useSelector(selectEntityById(clientId))
  const current = entity?.gender || ''
  const [isEditing, setIsEditing] = useState(false)
  const [isDropdownOpen, setIsDropdownOpen] = useState(false)

  const handleOptionSelect = (value: string) => {
    dispatch(updateGender({ id: clientId, value }))
    setIsEditing(false)
    setIsDropdownOpen(false)
  }

  const getCurrentDisplayValue = () => {
    const option = GENDER_OPTIONS.find(opt => opt.value === current)
