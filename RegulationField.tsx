...
import { selectEntityById, updateRegulation } from '@/state/entitiesSlice'
import { coerceBoolean, boolToStr, strToBool } from '@/lib/utils'
import { PencilIcon, CheckIcon, XMarkIcon, ChevronDownIcon } from '@heroicons/react/24/outline'

interface RegulationFieldProps {
  clientId: string
}

export function RegulationField({ clientId }: RegulationFieldProps) {
  const dispatch = useDispatch<AppDispatch>()
  const entity = useSelector(selectEntityById(clientId))
  const current = coerceBoolean(entity?.regulation)
  const [isEditing, setIsEditing] = useState(false)
  const [isDropdownOpen, setIsDropdownOpen] = useState(false)

  const handleOptionSelect = (value: string) => {
    const next = strToBool(value)
    dispatch(updateRegulation({ id: clientId, value: next }))
    setIsEditing(false)
    setIsDropdownOpen(false)
  }

  const getCurrentDisplayValue = () => {
    const val = boolToStr(current)
              ...
              {[
                ...
                <button
                  ...
                  className={`w-full px-3 py-2 text-left hover:bg-gray-50 transition-colors text-gray-600 ${
                    boolToStr(current) === option.value ? 'bg-blue-50' : ''
                  }`}
                  style={{ 
                    fontWeight: 400,
                    fontFamily: 'Poppins, sans-serif',
                ...
                >
                  <div className="flex items-center justify-between">
                    {option.label}
                    {boolToStr(current) === option.value && (
