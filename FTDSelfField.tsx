...
import { FTD_OPTIONS, normalizeFTD } from '@/config/ftd'

interface FTDSelfFieldProps {
  clientId: string
}

const FTD_OPTIONS_WITH_DEFAULT = [
  { value: '', label: 'Not Set' },
  ...FTD_OPTIONS.map(option => ({ value: option, label: option }))
]

export function FTDSelfField({ clientId }: FTDSelfFieldProps) {
  const dispatch = useDispatch<AppDispatch>()
  const entity = useSelector(selectEntityById(clientId))
  const current = normalizeFTD(entity?.ftdSelf || '')
  ...
  const getCurrentDisplayValue = () => {
    if (!current || current === '') return 'Not Set'
    return current
  }
      ...
      <div className="relative flex-1">
        ...
        {isDropdownOpen && (
          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
            <div className="py-1">
              {FTD_OPTIONS_WITH_DEFAULT.map((option) => (
                <button
                  key={option.value}
