...
import { emailTemplateColumns, EMAIL_TPL_TABLE_ID, initialColumnSizing } from './emailTemplateColumns'
import {
...
interface EmailTemplatesTableProps {
  searchQuery?: string
  onOpenAddModal: () => void
  onOpenEditModal: (template: EmailTemplate) => void
  onOpenPreviewModal: (template: EmailTemplate) => void
}
...
export default function EmailTemplatesTable({ 
  ...
  const allData = useSelector(selectAllEmailTemplates)
  ...
  const data = useMemo(() => {
    ...
    return allData.filter(template =>
      ...
      (template.language && template.language.toLowerCase().includes(query))
    )
  }, [allData, searchQuery])

  // Handle inline commit for editable fields
  const handleInlineCommit = (id: string, patch: Partial<Pick<EmailTemplate, 'name' | 'subject'>>) => {
    try {
      dispatch(updateTemplate({ id, patch }))
  ...
  const handleToggleActive = (id: string, isActive: boolean) => {
    try {
      dispatch(setTemplateActive({ id, isActive: !isActive }))
                ...
                >
                  {searchQuery.trim() ? (
                    <div className="space-y-2">
                      <p>No email templates match "{searchQuery}"</p>
