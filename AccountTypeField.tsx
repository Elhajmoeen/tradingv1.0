...
import { selectEntityById, updateAccountType } from '@/state/entitiesSlice'
...
export function AccountTypeField({ clientId }: AccountTypeFieldProps) {
  const dispatch = useDispatch<AppDispatch>()
  const entity = useSelector(selectEntityById(clientId))
  const current = entity?.accountType || ''
  const [isEditing, setIsEditing] = useState(false)
  const [isDropdownOpen, setIsDropdownOpen] = useState(false)
  
  // Use Redux-powered account types
  ...
  const handleOptionSelect = (value: string) => {
    dispatch(updateAccountType({ id: clientId, value }))
    setIsEditing(false)
    setIsDropdownOpen(false)
  }

  const getCurrentDisplayValue = () => {
    const option = ACCOUNT_TYPE_OPTIONS.find(opt => opt.value === current)
